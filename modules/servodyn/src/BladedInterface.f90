...
SUBROUTINE CallBladedDLL ( u, DLL, dll_data, p, ErrStat, ErrMsg )
   ...
   TYPE(ndarray)                :: avrSWAP_py
   TYPE(ndarray)                :: fromSC_py
   TYPE(ndarray)                :: toSC_py
   TYPE(ndarray)                :: aviFAIL_py
   TYPE(ndarray)                :: accINFILE_py
   TYPE(ndarray)                :: avcOUTNAME_py
   TYPE(ndarray)                :: avcMSG_py
   TYPE(tuple)                  :: args
   TYPE(object)                 :: return_value
   TYPE(ndarray)                :: return_array

   REAL, dimension(size(dll_data%avrSWAP))                         :: avrSWAP_COPIA_IN
   REAL, dimension(size(dll_data%SCoutput))                        :: toSC_COPIA_IN
   REAL, dimension(size(u%SuperController))                        :: fromSC_COPIA_IN
   REAL, dimension(size(dll_data%avrSWAP))                         :: avrSWAP_COPIA_OUT
   REAL, dimension(size(dll_data%SCoutput))                        :: toSC_COPIA_OUT
   ...
   print*, "callbladed", u%SuperController(1)
   print*, "avrSWAP INPUT: ", dll_data%avrSWAP

   avrSWAP_COPIA_IN = dll_data%avrSWAP
   toSC_COPIA_IN    = dll_data%SCoutput
   fromSC_COPIA_IN  = u%SuperController
   p%ierror = ndarray_create(avrSWAP_py, avrSWAP_COPIA_IN) !COPYING values to be given as input to DISCON not to modify the CallBladedDLL INPUTS received in OpenFAST
   p%ierror = ndarray_create(fromSC_py, fromSC_COPIA_IN)
   p%ierror = ndarray_create(toSC_py, toSC_COPIA_IN)

   p%ierror = tuple_create(args, 3)  !SETTING arguments to give to DISCON for turbine control
   p%ierror = args%setitem(0, avrSWAP_py)
   p%ierror = args%setitem(1, fromSC_py)
   p%ierror = args%setitem(2, toSC_py)

   p%ierror = call_py(return_value, p%DISCON, "DISCON", args) !CALLING CONTROL MODULE DISCON

   p%ierror = cast(return_array, return_value)
   p%ierror = return_array%get_data(matrix)

   DO k = 1, size(dll_data%avrSWAP) + size(dll_data%SCoutput)
      IF (k <= size(dll_data%avrSWAP)) THEN
         avrSWAP_COPIA_OUT(k) = matrix(k)
      ELSE
         toSC_COPIA_OUT(k-size(dll_data%avrSWAP)) = matrix(k)
      END IF
   ENDDO

   dll_data%avrSWAP = avrSWAP_COPIA_OUT
   dll_data%SCoutput = toSC_COPIA_OUT

   print*, "avrSWAP OUTPUT DISCON: ", avrSWAP_COPIA_OUT
   print*, "SC OUTPUT DISCON: ", toSC_COPIA_OUT
   ...
   call args%destroy !REMOVE all temporary data called in this cycle
   call avrSWAP_py%destroy
   call fromSC_py%destroy
   call toSC_py%destroy
   call aviFAIL_py%destroy
   call accINFILE_py%destroy
   call avcOUTNAME_py%destroy
   call avcMSG_py%destroy
   call return_value%destroy
   call return_array%destroy
...
SUBROUTINE BladedInterface_Init(u,p,m,y,InputFileData, ErrStat, ErrMsg)
   ...
   CALL AllocAry( m%dll_data%avrSwap,   R+(2*p%DLL_NumTrq)-1, 'avrSwap', ErrStat2, ErrMsg2 )
      ...
      print*, "size y % SuperController", SIZE(y%SuperController)    
      ...
      print*, "size m % dll % SuperController", SIZE(m%dll_data%SCoutput)    
   ...
   p%ierror = forpy_initialize()
   p%ierror = get_sys_path(p%paths)
   p%ierror = p%paths%append(".")
   p%ierror = import_py(p%DISCON, "DISCON") !LOADING DISCON
...
SUBROUTINE BladedInterface_End(u, p, m, ErrStat, ErrMsg)
   ...
   call p%DISCON%destroy
   call p%paths%destroy
...
!bjj: this assumes it is the value at the previous step; but we actually want the output GenTrq...              
   ...
   dll_data%avrSWAP(24) = u%YawRate
   ! --> CHANGING IT TO B: THE YAW RATE
   IF ( p%DLL_NumTrq == 0 )  THEN  ! Torque-speed table look-up not selected
   ...
   ! --> THIS RECORD WILL BE SET IN DISCON SO THAT I CAN OPERATE ALSO WITH TORQUE CONTROL
...
END IF
   ...
   dll_data%avrSWAP(37) = u%Yaw - p%NacYaw_North            !> * Record 37: Nacelle yaw angle from North (rad) [ \f$ u\%Yaw - p\%NacYaw\_North \f$ ]
   ! HERE u%YawAngle is always set to 0.0 --> wrong output
...
! Records 92-94 are outputs [see Retrieve_avrSWAP()]
   ...
   IF (p%AvgWindSpeed > 1000.0_ReKi) dll_data%avrSWAP(96) = u%HorWindV
...
!> * Record 46, demanded pitch rate (Collective pitch), is ingored since record 10 is set to 0 by ServoDyn indicating pitch position actuator
   ...
   dll_data%YawRateCom = dll_data%avrSWAP(48)       !> * Record 48: Demanded nacelle yaw rate (rad/s) OR Demanded nacelle yaw torque (Nm) if YAW TORQUE CONTROL ENABLED
