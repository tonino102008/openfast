...
SUBROUTINE IfW_UserWind_CalcOutput(Time, PositionXYZ, ParamData, Velocity, DiskVel, MiscVars, ErrStat, ErrMsg)
   ...
   DO i = 1, turb_loc(k(1)) - 1          !  EVALUATING SPEED AT TURBINES INLET AT (Y,Z) coord required by USER TO OBTAIN AddedTurbulence
      ...
      DO j = i + 1, turb_loc(k(1))  
         ...
         CALL WindCalc(Ct(i), gammat(i), a(i), ParamData%zhub(i), ParamData%drot(i), windentrypos(i,i), I0vec(i), ParamData%ka, &
   ...
   CONTAINS
      SUBROUTINE WindCalc(Ct, gamma, axind, z_hub, d_rot, Uinf_w, I0, ka, kb, alpha, beta, &
         ...
         REAL(ReKi),                             INTENT(IN   )  :: Ct, gamma, z_hub, d_rot, Uinf_w, axind
         ...
         !theta = (0.3 * gamma * (1 - ((1- Ct * COS(0.0)) ** 0.5))) / COS(0.0)  !REMEMBER EVERYTHING IN RADIANS
         x0 = ((d_rot * COS(gamma) / 2) * ((UR / (U_init + U0)) ** 0.5)) / ((U_init / (0.5*(U_init + U0))) * (alpha * I0 + beta * 0.5 * ((U_init - U0)/U_init)))
         theta = 0.6 * axind * gamma
         !theta = 0.6 * axind * COS(gamma)
         delta0 = x0 * TAN(theta)
         ...
         print*, "FAR WAKE X0 and Theta", x0, theta
