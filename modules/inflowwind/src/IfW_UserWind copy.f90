...
SUBROUTINE IfW_UserWind_CalcOutput(Time, PositionXYZ, ParamData, Velocity, DiskVel, MiscVars, ErrStat, ErrMsg)
   ...
   REAL(ReKi), DIMENSION(:),   ALLOCATABLE, SAVE               :: Ctin, gammain, gammainNF, gammainold, gammainNFold, ain !, I0vec  ! Turbine Parameters OpenFast order
   ...
   REAL(ReKi)                                                  :: tauF = (1/3) * ((2 * ACOS(-1.0)) / 1.2671)
   ...
   IF (.NOT. ALLOCATED(k)) THEN
      ...
      ALLOCATE(gammainold(SIZE(ParamData%Layout(:,1))))
      ALLOCATE(gammainNF(SIZE(ParamData%Layout(:,1))))
      ALLOCATE(gammainNFold(SIZE(ParamData%Layout(:,1))))
   ...
   IF (k(3) == 1) THEN
      ...
      END IF
      ! FOR THE WIND ANGLE --> YAW IS FILTERED TO KEEP A SMOOTH SIGNAL AVOIDING ISSUES WITH SOLVER --> MOMENTS ON BLADES ARE TOO SENSIBLE!!!
      !IF ((ABS(gammain(k(1))) - ABS(gammainold(k(1)))) < 0.01) gammain(k(1)) = gammainold(k(1)) !HACK TO KEEP WIND CONSTANT IN DIRECTION IF SMALL deltaYAW happens
      !gammain(k(1)) = (1/(2*tauF + ParamData%dT)) * ((2*tauF - ParamData%dT)*gammainold(k(1)) + ParamData%dT*(gammainNF(k(1)) + gammainNFold(k(1))))
      !gammainold(k(1)) = gammain(k(1))
      !gammainNFold(k(1)) = gammainNF(k(1))
   END IF
