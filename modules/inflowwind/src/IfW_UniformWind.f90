...
SUBROUTINE GetWindSpeed(InputPosition, p, m, op, WindSpeed, ErrStat, ErrMsg)
   ...
   REAL(ReKi)                                            :: kr                ! Terrain roughness for log wind profile
   ...
   !kr = 0.19 * ((op%VShr / 0.05) ** 0.07)
   !IF (InputPosition(3) > p%RefHt) THEN
   !   V1 = op%V * kr * ( LOG( InputPosition(3)/op%VShr ) &                                  ! power-law wind shear
   ...
   !      +  op%VGust                                                                        ! gust speed
   !ELSE
   !   V1 = op%V * kr * ( LOG( p%RefHt/op%VShr ) &                                  ! power-law wind shear
   !      + ( op%HShr   * ( InputPosition(2) * CosDelta + InputPosition(1) * SinDelta ) &    ! horizontal linear shear
   !      +  op%VLinShr * ( InputPosition(3) - p%RefHt ) )/p%RefLength  ) &                  ! vertical linear shear
   !      +  op%VGust                                                                        ! gust speed
   !END IF
