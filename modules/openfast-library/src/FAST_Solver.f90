...
!  TYPE(AD_OutputType),              INTENT(IN)     :: y_AD         !< AeroDyn outputs
   ...
   ELSEIF ( p_FAST%CompInflow == Module_OpFM )  THEN 
      ...
      if ( allocated(u_SrvD%SuperController) ) then
         print*, "u--->y_OpFM", y_OpFM%SuperController(1)
         print*, "u--->y_OpFM PRIMA", u_SrvD%SuperController(1)
         ! u_SrvD%SuperController = y_OpFM%SuperController
         y_OpFM%SuperController = u_SrvD%SuperController
         print*, "u--->y_OpFM DOPO", u_SrvD%SuperController(1)
      end if
...
if (n_t_global == 0) then   
   ...
   IF ( p_FAST%CompServo == Module_SrvD  ) THEN  
      print*, "Srvd_INPUTSolve in Calcoutputsandsolveforinputs"       
...
!...............................................................................................................................
   ...
   IF (p_FAST%CompInflow == Module_IfW) THEN
      ...
      CALL IfW_InputSolve( p_FAST, m_FAST, IfW%Input(1), IfW%p, AD14%Input(1), AD%Input(1), ED%Output(1), ErrStat2, ErrMsg2 )       
         ...
         ! WITH THIS LINE I COPY THE INPUTS FROM AERODYN TO INFLOW_WIND FOR WAKE CALCULATIONS
         IF (.NOT. ALLOCATED(IfW%m%UserWind%AllInAD)) ALLOCATE(IfW%m%UserWind%AllInAD(28)) ! 3bl * 9Points Axial Induction + 1 RtAeroCt
         IF (.NOT. ALLOCATED(IfW%m%UserWind%BlPos)) ALLOCATE(IfW%m%UserWind%BlPos(3,SIZE(AD%Input(1)%BladeProps(1)%BlSpn))) ! 3bl * 9Points Axial Induction Blades Position
         IF (.NOT. ALLOCATED(IfW%m%UserWind%BlOutNd)) ALLOCATE(IfW%m%UserWind%BlOutNd(SIZE(AD%Input(1)%BlOutNd))) ! 3bl * 9Points Axial Induction Blades Position
         IfW%m%UserWind%AllInAD(1:27) = AD%m%AllOuts(520:546)
         IfW%m%UserWind%AllInAD(28) = AD%m%AllOuts(1184)
         IfW%m%UserWind%BlPos(1,:) = AD%Input(1)%BladeProps(1)%BlSpn
         IfW%m%UserWind%BlPos(2,:) = AD%Input(1)%BladeProps(2)%BlSpn
         IfW%m%UserWind%BlPos(3,:) = AD%Input(1)%BladeProps(3)%BlSpn
         IfW%m%UserWind%BlOutNd = AD%Input(1)%BlOutNd
         IfW%m%UserWind%YawAngle = SrvD%m%dll_data%avrSWAP(37)
         ! END OF LINES
   ...
   IF ( p_FAST%CompServo == Module_SrvD  ) THEN
      ...
      ELSE
         print*, "Srvd_INPUT solve solve2 solve2"
      ...
      print*, "Srvd_Calcoutput solve solve2 Gli passo tutto sputtanato, vero?"
      print*, "Srvd_Calcoutput solve solve2 IL MIO INPUT ", Srvd%Input(1)%SuperController(1)
