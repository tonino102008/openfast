...
df1 = pd.read_csv('Bl1outin.txt')
bl1mom = df1.values
df2 = pd.read_csv('Bl2outin.txt')
bl2mom = df2.values
df3 = pd.read_csv('Bl3outin.txt')
bl3mom = df3.values
df4 = pd.read_csv('Azimuth.txt')
turbinfo = df4.values
df5 = pd.read_csv('/home/antonio/SOWFA/exampleCases/IDSET_1Turb_OBSERVER_COUPLED_DEMOD/5MW_Baseline/Wind/WindSim.uniform', sep='\t', header=None)
windinfo = df5.values
'''df6 = pd.read_csv('ECROSS.txt')
data6 = df6.values
df7 = pd.read_csv('EMOM.txt')
data7 = df7.values'''
...
plt.plot(time, numpy.multiply(bl1mom[:,0], numpy.cos(azimuth1)))
...
ind1 = numpy.random.randint(low = 0, high=len(m_out_time1), size=20000)
...
'''dfa = pd.read_csv('T.txt', sep='\t', header=None)
Tmat1 = dfa.values
Tmat = Tmat1[:,2:]
Vel = Tmat1[::2,1]
print(Tmat)
