...
def DISCON(avrSWAP_py, from_SC_py, to_SC_py):
    ...
    if iStatus >= 0 and aviFAIL_py >= 0:
        ...
        if not numpy.isclose(avrSWAP_py[36], globalDISCON.YawAngleGA) and globalDISCON.flagyaw == False:
            ...
            if abs(globalDISCON.PosYawRef - globalDISCON.YawAngleGA) > 0.0075:
                globalDISCON.VelYawRef = numpy.sign(globalDISCON.YawAngleGA - globalDISCON.YawAngleGAprev)*0.0349066/2
        ...
        if logic.counter == 1:
            ...
            if toNN.any() > 1:
                ...
                toNN[ii] = 1
                flagind = 1
            crosswindNN = NNEX.NN.forward(toNN) * vento_obs * NNEX.normY # VERSION WITHOUT YAW ANGLE
        else:
            crosswindNN = 0.0
