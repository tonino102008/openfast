...
df5 = pd.read_csv('/home/antonio/SOWFA/exampleCases/Floris_3Turb_SC/5MW_Baseline/Wind/WIND_FARM.FLORIS', sep=' ', header=None)
windinfo = df5.values[0:4,:]
...
R = 63   #TURBINE RADIUS
#print(windinfo)
V0 = windinfo[:,0] # it's a constant vector, so take only 1 value
kr = 0.19 * ((windinfo[:,1]/0.05) ** 0.07) * numpy.cos(-(windinfo[:,3]-270.0)*numpy.pi/180)
V0 = numpy.multiply(numpy.multiply(V0, kr), numpy.log(90.0/windinfo[:,1]))
yawerr = -(windinfo[:,3]-270.0)*numpy.pi/180
vert_shear = numpy.array([0.01, 0.01, 0.01, 0.01])
u0_p = numpy.array([0, 0, 0, 0]) #CROSS_WIND
...
V0a = numpy.array([10, 10, 10, 10]) # it's a constant vector, so take only 1 value
yawerr = -(windinfo[:,3]-270.0)*numpy.pi/180
vert_shear = numpy.array([0.01, 0.01, 0.01, 0.01])
V0a_int = inter.interp1d(timewind, V0)
...
dfa = pd.read_csv('Tmatrices_FLORIS.txt', sep='\t', header=None)
Tmat1 = dfa.values
Tmat = Tmat1[:,2:]
Vel = Tmat1[::2,1]
print(Tmat)
...
plt.plot(data6[:,4], data6[:,0], 'b', data6[:,4], data6[:,1], 'g', time, u0_tot,'r', datadata[:,0], datadata[:,2], 'r--')
plt.title("CROSS WIND ESTIMATE2")
plt.ylim(-5,5)
plt.show()


plt.plot(data6[:,4], numpy.arctan2(data6[:,0], data8[:,0])*180/numpy.pi, 'b', data6[:,4], numpy.arctan2(data6[:,1], data8[:,0])*180/numpy.pi, 'g', datadata[:,0], numpy.arctan2(datadata[:,2], datadata[:,1])*180/numpy.pi,'r')
