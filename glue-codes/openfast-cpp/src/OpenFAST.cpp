  ...
  if (!dryRun) {
    ...
    // }
     ...
     if(scStatus) {
       ...
       std::cout << nt_global << " NT_GLOBAL\n";
       sc->init(nTurbinesGlob, numScInputs, numScOutputs, dtFAST, &nt_global);
...
void fast::OpenFAST::step() {
  ...
  ********************************* */
   ...
   for (int iTurb=0; iTurb < nTurbinesProc; iTurb++) {
     ...
     std::cout << "FAST_OpFM_STEP " << iTurb << std::endl ;     
  ...
  if ( (((nt_global - ntStart) % nEveryCheckPoint) == 0 )  && (nt_global != ntStart) ) {
    ...
    if(scStatus) {
      if (fastMPIRank == 0) {
          std::cout << "Checkpoint write restart file"  << std::endl ;
...
void fast::OpenFAST::fillScInputsGlob() {
  ...
  for(int iTurb=0; iTurb < nTurbinesGlob; iTurb++) {
    ...
    std::cout << "Turbine Glob " << nTurbinesGlob << std::endl ;
    std::cout << "Turbine Proc " << nTurbinesProc << std::endl ;
    std::cout << "sc InputsGlob size:  " << scInputsGlob.size() << std::endl ;


  for(int iTurb=0; iTurb < nTurbinesProc; iTurb++) {
    for(int iInput=0; iInput < numScInputs; iInput++) {
      std::cout << cDriverSC_Input_from_FAST[iTurb].toSC[iInput]  << " OF: riempito input Locali fromSC" << std::endl ;
      scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput] = cDriverSC_Input_from_FAST[iTurb].toSC[iInput] ;
      std::cout << scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput]  << " OF: riempito input Locali fromSC" << std::endl ;
    }
    if (iTurb == nTurbinesProc -1) {
      std::cout << " SC RIEMPITO!!" << std::endl ;
    }
  }
  ...
  for(int iTurb=0; iTurb < nTurbinesProc; iTurb++) {
    for(int iInput=0; iInput < numScInputs; iInput++) {
      std::cout << scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput]  << " OF: riempito input Locali fromSC PRINT PASSAGE" << std::endl ;
    }
  }

  // CHECKING FOR POSSIBLE INTEGER
  for(int iTurb=0; iTurb < nTurbinesProc; iTurb++) {
    for(int iInput=0; iInput < numScInputs; iInput++) {
      if(floor(scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput])==ceil(scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput])) {
        std::cout << " INTEGER DETECTED" << std::endl ;
        double support = static_cast<double>(scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput]) + 0.01;
        scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput] = support;
      }
      std::cout << scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput]  << " OF: riempito input Locali fromSC PRINT PASSAGE" << std::endl ;
    }
  }
  ...
  // CHECKING FOR POSSIBLE MISSING VARIABLES
  for(int iTurb=0; iTurb < nTurbinesProc; iTurb++) {
    for(int iInput=0; iInput < numScInputs; iInput++) {
        if ((scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput] == NULL) || scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput] != scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput]) {
          scInputsGlob[turbineMapProcToGlob[iTurb]*numScInputs + iInput] = 0.0;
          std::cout << " NON C'ERA NULLA NEL VETTORE!!! FORSE ERRORE ERA QUI" << std::endl ;
    }    }
  }
  
  std::cout << " MPI CONTROL " << std::endl ;
  if (MPI_COMM_NULL != fastMPIComm) {
    std::cout << " MPI CONTROL 0 " << std::endl ;
    MPI_Allreduce(MPI_IN_PLACE, scInputsGlob.data(), numScInputs*nTurbinesGlob, MPI_DOUBLE, MPI_SUM, fastMPIComm) ;
    std::cout << " MPI CONTROL 1 " << std::endl ;
  }
...
void fast::OpenFAST::fillScOutputsLoc() {
  ...
  // Fills the local array containing outputs from the supercontroller to each turbine
    ...
    std::cout << "sc OutputsGlob size:  " << scOutputsGlob.size() << std::endl ;


  for(int iTurb=0; iTurb < nTurbinesProc; iTurb++) {
     std::cout << "Turbine " << iTurb << std::endl ;
    for(int iOutput=0; iOutput < numScOutputs; iOutput++) {
      cDriverSC_Output_to_FAST[iTurb].fromSC[iOutput] = scOutputsGlob[turbineMapProcToGlob[iTurb]*numScOutputs + iOutput] ;
      std::cout << "Problema qui" << std::endl;
      float tmp_val = (float) scOutputsGlob[turbineMapProcToGlob[iTurb]*numScOutputs + iOutput] ;
      std::cout << "FLOAT VALUE " << tmp_val << " var di tipo " << typeid(tmp_val).name() << std::endl;
      std::cout << "SC_LENGHT OpenFOAM" << cDriver_Output_to_FAST[iTurb].SuperController_Len << std::endl;
      cDriver_Output_to_FAST[iTurb].SuperController = &tmp_val;
      std::cout << "cDriver_Output_to_FAST[iTurb].SuperController" << cDriver_Output_to_FAST[iTurb].SuperController << std::endl;
      std::cout << iOutput  << "output " << std::endl ;
      std::cout << cDriverSC_Output_to_FAST[iTurb].fromSC[iOutput]  << "OF: riempito output Locali fromSC" << std::endl ;
    }
  }
