...
void fast::OpenFAST::allocateMemory() {
  ...
  if (dryRun) {
    if (nTurbinesProc > 0) {
      ...
      oss << "turbineAlloc." << worldMPIRank << ".txt";
      std::string An = oss.str();
      turbineAllocFile.open(An.c_str()) ;
...
void fast::OpenFAST::readVelocityData(int nTimesteps) {
  ...
  oss1 << "velDatafile." << worldMPIRank << ".h5";
  std::string An1 = oss1.str();
  hid_t velDataFile = H5Fopen(An1.c_str(), H5F_ACC_RDWR, H5P_DEFAULT);
  ...
  for (int iTurb=0; iTurb < nTurbines; iTurb++) {
    ...
    oss2 << "/turbine" << iTurb;
    std::string An2 = oss2.str();
    hid_t dset_id = H5Dopen2(velDataFile, An2.c_str(), H5P_DEFAULT);
...
hid_t fast::OpenFAST::openVelocityDataFile(bool createFile) {
  ...
  if (createFile) {
    ...
    oss3 << "velDatafile." << worldMPIRank << ".h5";
    std::string An3 = oss3.str();
    velDataFile = H5Fcreate(An3.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    ...
    for (int iTurb = 0; iTurb < nTurbinesProc; iTurb++) {
      ...
      oss4 << "/turbine" << iTurb;
      std::string An4 = oss4.str();
      hid_t dataSet = H5Dcreate(velDataFile, An4.c_str(), H5T_NATIVE_DOUBLE, dataSpace, H5P_DEFAULT, dcpl_id, H5P_DEFAULT);    
    ...
    oss5 << "velDatafile." << worldMPIRank << ".h5";
    std::string An5 = oss5.str();
    velDataFile = H5Fopen(An5.c_str(), H5F_ACC_RDWR, H5P_DEFAULT);
  }
...
void fast::OpenFAST::backupVelocityDataFile(int curTimeStep, hid_t & velDataFile) {
    ...
    oss6 << "velDatafile." << worldMPIRank << ".h5";
    std::string An6 = oss6.str();
    std::ostringstream oss7;
    oss7 << "velDatafile." << worldMPIRank << ".h5. " << curTimeStep << ".bak";
    std::string An7 = oss7.str();
    std::ifstream source(An6.c_str(), std::ios::binary);
    std::ofstream dest(An7.c_str(), std::ios::binary);
...
void fast::OpenFAST::writeVelocityData(hid_t h5File, int iTurb, int iTimestep, OpFM_InputType_t iData, OpFM_OutputType_t oData) {
  ...
  oss8 << "/turbine" << iTurb;
  std::string An8 = oss8.str();
  hid_t dset_id = H5Dopen2(h5File, An8.c_str(), H5P_DEFAULT);
  hid_t dspace_id = H5Dget_space(dset_id);
  H5Sselect_hyperslab(dspace_id, H5S_SELECT_SET, start, NULL, count, NULL);
  hid_t mspace_id = H5Screate_simple(3, count, NULL);  
